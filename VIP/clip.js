...
function parseEstimatedSize(sizeStr) {
    if (!sizeStr) return 0;
    const match = sizeStr.match(/([\d.]+)\s*Mo/i);
    if (match) {
...
async function handleVideoDownload(senderId, video, quality = '360p', format = 'MP4') {
    try {
        ...
        const estimatedSizeStr = video.Taille_estimee?.[quality] || '';
        const estimatedSize = parseEstimatedSize(estimatedSizeStr);
...
📊 Qualité : ${quality}
📁 Format : ${formatLabel}
📦 Taille estimée : ${estimatedSizeStr || 'N/A'}
Préparation...
        `.trim());
        ...
        console.log('Taille estimée:', estimatedSizeStr, '=', estimatedSize, 'bytes');
        
        if (format === 'MP3') {
            let audioSize = await getVideoSize(downloadUrl);
            if (audioSize === 0 && estimatedSize > 0) {
                audioSize = estimatedSize * 0.3;
            }
            const sizeMB = (audioSize / (1024 * 1024)).toFixed(2);
            
            console.log(`Taille audio MP3 (${quality}): ${sizeMB} MB`);
            
            const sizeInfo = audioSize > 0 ? `${sizeMB} MB` : estimatedSizeStr || 'N/A';
            const canSendDirect = audioSize > 0 && audioSize < MAX_DIRECT_SEND_SIZE;
            
            await sendMessage(senderId, `
📦 Taille: ${sizeInfo}
📤 ${canSendDirect ? 'Envoi de l\'audio en pièce jointe...' : 'Préparation du lien...'}
            `.trim());
...
🔄 Tapez "clip" pour une nouvelle recherche
            ...
            let videoSize = await getVideoSize(downloadUrl);
            if (videoSize === 0 && estimatedSize > 0) {
                videoSize = estimatedSize;
            }
            const sizeMB = (videoSize / (1024 * 1024)).toFixed(2);
            
            console.log(`Taille vidéo (${quality}): ${sizeMB} MB, estimée: ${estimatedSizeStr}`);
            
            const sizeInfo = videoSize > 0 ? `${sizeMB} MB` : estimatedSizeStr || 'N/A';
            const canSendDirect = videoSize > 0 && videoSize < MAX_DIRECT_SEND_SIZE;
            
            await sendMessage(senderId, `
📦 Taille: ${sizeInfo}
📤 ${canSendDirect ? 'Envoi de la vidéo en pièce jointe...' : 'Préparation du lien...'}
            `.trim());
            ...
            if (canSendDirect) {
                ...
                console.log(`Vidéo trop volumineuse (${sizeMB} MB) ou taille inconnue, envoi du lien uniquement`);
            }
